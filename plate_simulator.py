...
class PlateSimulator:
    ...
    # Background:      dark grey plastic   →  BGR (58, 56, 52)
    # Well wall ring:  lighter grey ring   →  BGR (112, 110, 106)
    PURPLE_BGR: np.ndarray = np.array([170, 35, 155], dtype=np.float32)
    BLUE_BGR:   np.ndarray = np.array([200, 120,  40], dtype=np.float32)
    BG_BGR:     np.ndarray = np.array([ 58,  56,  52], dtype=np.float32)
    WALL_BGR:   np.ndarray = np.array([112, 110, 106], dtype=np.float32)
    ...
    def generate_image(self, well_labels: np.ndarray) -> np.ndarray:
        ...
        for row_i in range(self.ROWS):
            for col_i in range(self.COLS):
                ...
                is_purple = bool(well_labels[row_i, col_i])
                self._draw_well(img, cx, cy, is_purple)
    ...
    def _make_background(self) -> np.ndarray:
        ...
        # Subtle plastic-surface texture
        texture = self.rng.normal(0, 3, img.shape).astype(np.float32)
        img += texture
        # Slightly brighter plate interior (raised edge effect)
        r = self.well_radius
    ...
    def _apply_illumination_gradient(self, img: np.ndarray):
        """Simulate non-uniform illumination (brighter in centre)."""
        cx, cy = self.W / 2.0, self.H / 2.0
        dist = np.sqrt((self._xx - cx) ** 2 + (self._yy - cy) ** 2)
        max_d = np.sqrt(cx ** 2 + cy ** 2)
        gain = 1.0 + 0.22 * (1.0 - dist / max_d)           # 1.22 at centre → 0.78 at corner
        img *= gain[:, :, np.newaxis]

    def _draw_well(self, img: np.ndarray, cx: int, cy: int, is_purple: bool):
        ...
        base = self.PURPLE_BGR.copy() if is_purple else self.BLUE_BGR.copy()
